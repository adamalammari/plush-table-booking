  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        scrolled
          ? 'bg-background/95 backdrop-blur-xl border-b border-border py-3 elegant-shadow'
          : 'bg-transparent py-5'
      }`}
    >
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        <h2 className="text-xl font-display text-primary font-bold tracking-wide">المائدة الذهبية</h2>
        ...
        <div className="flex items-center gap-3">
          <Link to="/login" className="hidden md:block">
            <Button variant="ghost" size="sm" className="text-foreground/40 text-xs hover:text-primary">
              لوحة التحكم
            </Button>
          </Link>
          <Button
            onClick={() => navigate('/reserve')}
            className="hidden md:flex gold-gradient text-background font-semibold text-sm px-6 hover:opacity-90 transition-opacity"
          >
            احجز الآن
          </Button>
          <button
          ...
          >
            <div className="px-6 py-6 flex flex-col gap-4">
              ...
              <Button
                ...
                className="gold-gradient text-background font-semibold mt-2"
              >
                احجز الآن
              </Button>
            </div>
